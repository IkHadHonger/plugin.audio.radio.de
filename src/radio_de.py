...
class RadioDE():
    ...
    def _unify_stations(self, data):
        ...
        if data:
            stations = data.get('playables', []) or []
            for item in stations:
                # The current radio.net API may return entries without a
                # ``streams`` key (or with an empty stream list).  Older
                # versions of this add-on assumed streams[0] always existed,
                # which made paginated lists fail with KeyError: 'streams'.
                streams = item.get('streams') or []
                stream_url = ''

                # Prefer a stream explicitly marked VALID. If no status is
                # supplied, use the first stream that has a URL.
                for stream in streams:
                    if not isinstance(stream, dict) or not stream.get('url'):
                        continue
                    if stream.get('status') == 'VALID':
                ...
                if not stream_url:
                    xbmc.log('[Radio.de] Skipping station without usable stream: {}'.format(item.get('name', item.get('id', 'unknown'))), xbmc.LOGWARNING)
                    continue

                record = {'data': {}, 'mode': ''}
                record['mode'] = 'play_stream'
                record['data']['id'] = item.get('id', '')
                record['data']['name'] = item.get('name', '')
                record['data']['icon_url'] = item.get('logo300x300', '')
                record['data']['stream_url'] = stream_url
                result.append(record)
